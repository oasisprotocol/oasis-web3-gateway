...
func benchmarkMain(cmd *cobra.Command, args []string) {
	...
	cfg.ChainID, _ = cmd.Flags().GetUint64(cfgChainID)
	...
	if cfg.Web3URL, err = url.Parse(gatewayURL); err != nil {
		logger.Error("failed to parse gateway URL", "err", err)
		os.Exit(1)
	}
